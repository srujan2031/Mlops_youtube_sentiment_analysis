...
> Your folders may differ slightly — commands below are Windows-friendly and work from the project root.
...
## ⚙️ Prerequisites
- **Python 3.11** (recommended). Python **3.13** works if you keep **mlflow-skinny** and remove full **mlflow**.
- **Windows PowerShell** (commands below use it)
- (Optional) **AWS CLI** and an S3 bucket if you want DVC remote.
...
for pkg in ["punkt", "stopwords", "wordnet", "omw-1.4", "vader_lexicon"]:
    ...
    except Exception as e: print(f"NLTK download failed for {pkg}: {e}")
PY
```
...
```powershell
# Remove full MLflow pin; keep mlflow-skinny
...
# Reproduce pipeline (ingest → preprocess → train → evaluate)
dvc repro
...
# Find files that have a main() or training code
Get-ChildItem -Recurse -Filter *.py | Select-String -Pattern 'if __name__ == "__main__"' | Select-Object Path, LineNumber, Line

# Then run the script that performs training, e.g.:
python src\models\train.py   # adjust the path/filename if different
```
...
This repo includes **Flask** in `requirements.txt`. Detect the app file and run it:

```powershell
# Find a Flask entry (file that imports/creates Flask)
...
## 🔧 Configuration
- **YouTube API key** (only if ingestion fetches live comments)
  ```powershell
